      ...
      const [massIntention] = await prisma.$transaction(async (tx) => {
        ...
        await tx.mass.update({
          ...
          data: {
            ...
            status:
              mass.availableIntentionsSlots - 1 <= 0 &&
...
export async function updateMassIntentionStatus(
  ...
  try {
    ...
    await logActivity({
      ...
      action: `Your mass intention for ${
        massIntention.name
    ...
    try {
      await prisma.$transaction(async (tx) => {
        ...
        await tx.mass.update({
          ...
          data: {
            availableIntentionsSlots:
              massIntention.mass.availableIntentionsSlots + 1,

...
export default async function AdminMassIntentionsPage() {
  ...
  const pendingIntentions = massIntentions.filter(
    (intention) => intention.status === "PENDING"
  );
  const approvedIntentions = massIntentions.filter(
    (intention) => intention.status === "APPROVED"
  );
  const rejectedIntentions = massIntentions.filter(
    (intention) => intention.status === "REJECTED"
  );
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-8">
          <Card>
            <CardHeader>
              <CardTitle>
                Pending Requests ({pendingIntentions.length})
              </CardTitle>
            </CardHeader>
            <CardContent>
              {pendingIntentions.length > 0 ? (
                ...
                <p className="text-gray-500 text-center py-4">
                  No pending mass intention requests.
                </p>
              )}
            </CardContent>
          </Card>
        ...
        <div className="mb-8">
          <Card>
            <CardHeader>
              <CardTitle>
                Approved Requests ({approvedIntentions.length})
              </CardTitle>
            </CardHeader>
            <CardContent>
              {approvedIntentions.length > 0 ? (
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                      {approvedIntentions.map((intention) => (
                        <tr key={intention.id}>
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                            <form
                              action={deleteMassIntention.bind(
                                null,
                                intention.id
                              )}
                            >
                ...
                <p className="text-gray-500 text-center py-4">
                  No approved mass intention requests.
                </p>
              )}
            </CardContent>
          </Card>
        ...
        <div>
          <Card>
            <CardHeader>
              <CardTitle>
                Rejected Requests ({rejectedIntentions.length})
              </CardTitle>
            </CardHeader>
            <CardContent>
              {rejectedIntentions.length > 0 ? (
                ...
                <p className="text-gray-500 text-center py-4">
                  No rejected mass intention requests.
                </p>
              )}
            </CardContent>
          </Card>
